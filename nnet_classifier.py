...
class Classification:
    ...
    def validate(self, X, y):
        ...
        y = y.astype(int)
        self.target = CategoricalFeatures(verbose=False)
        ...
        if self.tune:
            ...
            for i in range(grid.shape[0]):
                ...
                if self.deep:
                    ...
                    width = 2
                trainy_target = self.target.fit_transform(trainy)
                self.nnet = self.build(
                    inputs=trainX.shape[1], 
                    outputs=trainy_target.shape[1], 
                    hidden=np.repeat(grid["nodes"][i], width), 
                    learning_rate=grid["learning_rate"][i], 
                    l1_penalty=0,
                )
                self.nnet.fit(trainX, trainy_target, epochs=50, batch_size=16, workers=-1, verbose=0)

                # score the neural network
                predictions = np.argmax(self.nnet.predict(testX, verbose=0), axis=1)
            ...
            grid = grid.sort_values(by="accuracy", ascending=False).reset_index(drop=True)
            ...
            if self.deep:
                ...
                width = 2
            y_target = self.target.fit_transform(y)
            self.nnet = self.build(
                inputs=X.shape[1], 
                outputs=y_target.shape[1], 
                hidden=np.repeat(nodes, width), 
                learning_rate=learning_rate, 
                l1_penalty=0,
            )
            self.nnet.fit(X, y_target, epochs=500, batch_size=16, workers=-1, verbose=0)
        else:
    ...
    def predict(self, X):
        ...
        y = np.argmax(self.nnet.predict(X, verbose=0), axis=1)
    ...
    def performance(self, X, y):
        ...
        predictions = np.argmax(self.nnet.predict(X, verbose=0), axis=1)
        y = y.iloc[:,0].to_numpy()
    ...
    def importance(self, X, y):
        ...
        baseline = accuracy_score(
            ...
            y_pred=np.argmax(self.nnet.predict(X, verbose=0), axis=1),
        )
        ...
        for col in X.columns:
            for i in range(iterations):
                ...
                accuracy.append(accuracy_score(  # prediction error
                    y_true=y.iloc[:,0].to_numpy(),
                    y_pred=np.argmax(self.nnet.predict(X_perturb, verbose=0), axis=1),
                ))
            
            # feature importance
            accuracy = np.mean(accuracy)
            change = accuracy / baseline - 1  # if the accuracy changes a lot then it must be an important feature
